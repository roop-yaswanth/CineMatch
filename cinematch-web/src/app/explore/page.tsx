...
function ExplorePageInner() {
  ...
  const handleMovieAction = useCallback(
    async (m: MovieLike | DetailMovie, action: "love" | "like" | "dislike" | "watchlist" | "skip" | "remove" | "remove_rating" | "remove_watchlist") => {
      if (!session) return;
      const targetId = ("tmdb_id" in m && m.tmdb_id) ? m.tmdb_id : (m as { id: number }).id;
      if (action !== "skip") {
        setUserReactions((prev) => {
          const cur = prev[targetId] || {};
          if (action === "watchlist") return { ...prev, [targetId]: { ...cur, watchlist: true } };
          if (action === "remove_watchlist") return { ...prev, [targetId]: { ...cur, watchlist: false } };
          if (action === "remove_rating") return { ...prev, [targetId]: { ...cur, rating: undefined } };
          if (action === "remove") {
            const next = { ...prev };
  ...
  const handleAction = useCallback(
    (action: "love" | "like" | "dislike" | "watchlist" | "skip" | "remove" | "remove_rating" | "remove_watchlist") => active && void handleMovieAction(active, action),
